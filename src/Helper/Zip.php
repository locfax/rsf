...
class Zip {
    ...
    public function dozip($dir, $zipfilename) {
        if (function_exists('gzcompress')) {
            $curdir = getcwd();
            if (is_array($dir)) {
                $filelist = $dir;
            } else {
                $filelist = $this->_GetFileList($dir);
            }
            if ((!empty($dir)) && (!is_array($dir)) && (is_dir($dir))) {
                chdir($dir);
            } else {
                chdir($curdir);
            }
            if (count($filelist) > 0) {
                foreach ($filelist as $filename) {
                    if (is_file($filename)) {
                        ...
                        $this->_addFile($content, $filename);
                    }
                }
                $out = $this->_file();
                chdir($curdir);
            ...
            while ($files = readdir($dh)) {
                if ("." != $files && ".." != $files) {
                    if (is_dir($dir . $files)) {
                        $curdir = getcwd();
                        chdir($dir . $files);
                        $file = array_merge($file, $this->_GetFileList("", "$pref$files/"));
                        chdir($curdir);
                    } else {
                        $file[] = $pref . $files;
                    }
