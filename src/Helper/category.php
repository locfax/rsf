...
function div_class($data, $level = 0, $controller = 'class', $extra = '', $idname = 'catid', $pidname = 'upid', $name = 'name', $fchildrens = 'children') {
    ...
    foreach ($data as $row) {
        ...
        $string .= $row[$name] . '</span>' . _option_href($row[$idname], $row[$name], $controller, $extra, $row[$pidname]) . '</li>';
        if (isset($row[$fchildrens])) {
            $string .= div_class($row[$fchildrens], $level + 1, $controller, $extra, $idname, $pidname, $name, $fchildrens);
        }
    }
    return $string;
}
...
function div_faqs($data, $level = 0, $idname = 'id', $name = 'title', $fchildrens = 'children') {
    foreach ($data as $row) {
        ...
        $string = str_repeat('|--', $level) . '<img src="assets/img/icons/bullet.gif"/>(ID:' . $row[$idname] . ') ' . $row[$name] . _faq_href($row[$idname], $row[$name]) . '<br/>';
        echo $string;
        if (isset($row[$fchildrens])) {
            $level += 1;
            div_faqs($row[$fchildrens], $level);
        }
    }
}
...
function select_faqs($data, $fpid = 0, $level = 0, $idname = 'id', $name = 'title', $fchildrens = 'children') {
    ...
    foreach ($data as $row) {
        ...
        if (isset($row[$fchildrens])) {
            $level += 1;
            select_faqs($row[$fchildrens], $fpid, $level);
        }
    }
}
